...
module.exports = async (req, res, next) => {
  ...
  try {
    ...
    if (token.success != true && token.access != 'student' && (token.type != 'access_token' && token.type != 'refresh_token')) return res.status(401).json(AuthError);
    ...
    if (token.access != 'student') return res.status(401).json(AuthError);
